...
class QoS(object):
	...
	def _getPortsWithQueue(self):
		cmd = 'sudo dpctl unix:'+self.unix_socket+' stats-queue'
	...
	def _getQueueConfigs(self,portsList):
		result=[]
		prev = -1
		for port,queue_id in portsList:
			current = port
			if (current != prev):
				cmd = 'sudo /usr/local/bin/dpctl unix:'+self.unix_socket+' queue-get-config %d' %int(port)
				p = Popen([cmd],shell=True, stdin=PIPE, stdout=PIPE, stderr=PIPE)
				output,err = p.communicate()
				if err:
					print err
				...
				for queueID,minrate in output:
					...
					adict={}
					if self.queues_max_rate.has_key(port+','+queueID):
						maxrate = self.queues_max_rate[port+','+queueID]
						adict = {"queue_id":queueID,"min_rate":minrate,"max_rate":maxrate}
					else:
	...
	def set_queue(self, rest, vlan_id):
		...
		if self.unix_socket is None:
			msg = {'result': 'failed',
			   'details': 'unix socket/switch address is not set'}
			return REST_COMMAND_RESULT, msg
